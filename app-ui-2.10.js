...
async function loadFamilyData(){
  ...
  familyPlayers=(profile?.role==='player'||profile?.accountType==='player')&&profile?.playerId
    ? fps.filter(p=>p.id===profile.playerId)
    : fps;
...
function renderUserAdmin(){
  if(profile?.role!=='admin'){
    $('#userAdminBody').innerHTML='<tr><td colspan="7">Solo los Administradores pueden gestionar roles.</td></tr>';
    return;
  }
  ...
  $('#userAdminSummary').innerHTML=[
    ...
    [allUsers.filter(u=>effectiveUserRole(u)==='player').length,'Jugadoras'],
    [allUsers.filter(u=>['trainer','assistant'].includes(effectiveUserRole(u))).length,'Personal deportivo'],
    [allUsers.filter(u=>(u.status||'active')==='active').length,'Activos']
  ].map(([v,l])=>`<article class="stat"><strong>${v}</strong><span>${l}</span></article>`).join('');
  $('#userAdminBody').innerHTML=list.map(u=>`<tr>
    <td><strong>${esc(u.fullName||'—')}</strong><div class="muted">${esc(u.accountType||'')}</div></td>
    <td>${esc(u.email||'')}</td><td>${esc(u.phone||'')}</td>
    <td><span class="badge neutral">${esc(userRoleLabel(u))}</span></td>
    <td>${players.filter(p=>(p.linkedUserIds||[]).includes(u.id)).map(p=>`<button class="chip chip-button" data-view-player="${p.id}">${esc(p.name)}</button>`).join('')||'—'}</td>
    <td><span class="badge ${u.status||'active'}">${esc(statusLabel(u.status||'active'))}</span></td>
    <td><button class="btn secondary" data-manage-role="${u.id}" type="button">Gestionar rol</button></td>
  </tr>`).join('')||'<tr><td colspan="7">No hay usuarios para los filtros seleccionados.</td></tr>';
}
function renderFamilyHome(){if(state.familyDataLoading){setFamilyLoading(true);return}const bal=familyCharges.filter(c=>['pending','partial'].includes(c.status)).reduce((s,c)=>s+Number(c.amount||0)-Number(c.paidAmount||0),0);$('#familyWelcome').textContent=`Bienvenido, ${profile?.fullName||''}`;$('#familyPlayers').innerHTML=familyPlayers.map(p=>`<article class="panel athlete-card clickable-card" data-view-player="${p.id}"><span class="eyebrow">${esc(p.playerCode)}</span><h3>${esc(p.name)}</h3><p><strong>${esc(catNames(p))}</strong></p><button class="btn secondary" data-view-player="${p.id}">Ver perfil</button></article>`).join('')||'<article class="panel"><h3>Sin jugadoras vinculadas</h3><p>Tu información ya terminó de cargar. Puedes enviar una solicitud desde “Vincular jugadora”.</p></article>';$('#familyUpcoming').innerHTML=familyEvents.filter(e=>e.date>=today()&&e.status!=='cancelled').slice(0,5).map(eventMini).join('')||'<p class="muted">No hay eventos próximos.</p>';const isPlayer=profile?.accountType==='player'||profile?.role==='player';$('#familyBalancePanel').classList.toggle('hidden',isPlayer);if(!isPlayer)$('#familyBalance').innerHTML=`<div class="big-number">${money(bal)}</div><p class="muted">Saldo total pendiente de las jugadoras vinculadas.</p>`}
...
function showApp(user,p){
  ...
  else{state.familyDataLoading=true;go('familyHome');loadFamilyData().then(()=>{startFamilyRealtime();startNotificationRealtime();}).catch(e=>alert(err(e)));const payBtn=$('[data-view="familyPayments"]');if(payBtn)payBtn.classList.toggle('hidden',p.accountType==='player'||p.role==='player');}
}
