...
function trainerEventCard(e){
  ...
  return `<article class="panel trainer-event-card" data-trainer-event="${e.id}" role="button" tabindex="0">
    ...
    <p class="muted">${esc(venueName(e.venueId))} · Uniforme: ${esc(uniformLabel(e.uniform))}${locationUrl?' · Ubicación disponible':''}</p>
  </article>`;
}
...
$('#eventForm').onsubmit=async e=>{
  ...
  try{
    ...
    const data={orgId:ORG_ID,categoryId,seasonId:$('#eventSeason').value,type:$('#eventType').value,title:$('#eventTitle').value.trim(),opponent:$('#eventOpponent').value.trim(),date:$('#eventDate').value,startTime:$('#eventStart').value,endTime:$('#eventEnd').value,homeAway:$('#eventHomeAway').value,venueId:$('#eventVenue').value,locationUrl:$('#eventLocationUrl').value.trim(),uniform:$('#eventUniform').value,status:$('#eventStatus').value,notes:$('#eventNotes').value.trim(),updatedAt:serverTimestamp()};
    if(id){
      const existing=events.find(x=>x.id===id)||trainerEvents.find(x=>x.id===id);
      if(coaching&&(!existing||!(profile.assignedCategoryIds||[]).includes(existing.categoryId)))return alert('No tienes permiso para editar este evento.');
      await updateDoc(doc(db,'events',id),data);
    }else{
      await addDoc(collection(db,'events'),{...data,createdBy:auth.currentUser.uid,createdByRole:profile?.role||'',createdAt:serverTimestamp()});
    }
    $('#eventDialog').close();
