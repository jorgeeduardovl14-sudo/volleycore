...
function showApp(user,p){
  ...
  else{state.familyDataLoading=true;go('familyHome');loadFamilyData().then(()=>{startFamilyRealtime();startNotificationRealtime();}).catch(e=>alert(err(e)));const playerMode=isPlayerProfile(p);const payBtn=$('[data-view="familyPayments"]');if(payBtn)payBtn.classList.toggle('hidden',playerMode);const linkBtn=$('[data-view="familyLink"]');if(linkBtn)linkBtn.classList.toggle('hidden',playerMode);}
}
