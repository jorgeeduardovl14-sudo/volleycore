...
function categoryUsedNumbers(categoryId){return [...new Set(players.filter(p=>p.status!=='inactive'&&playerCatIds(p).includes(categoryId)&&String(p.number??'').trim()!=='').map(p=>Number(p.number)).filter(Number.isFinite))].sort((a,b)=>a-b)}
...
function addMonths(month,delta){const[y,m]=month.split('-').map(Number),d=new Date(y,m-1+delta,1);return`${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,'0')}`}
function latestKnownMonthForPlayer(playerId){const months=familyCharges.filter(c=>c.playerId===playerId).map(c=>c.month).filter(Boolean).sort();return months.at(-1)||monthNow()}
function selectedBulkChargeIds(){return $$('#sinpeChargeList input[type="checkbox"]:checked').map(x=>x.value)}
function futurePrepayAllocations(){const out=[];$$('.sinpe-prepay-row').forEach(row=>{const playerId=row.dataset.playerId,count=Math.max(0,Number(row.querySelector('input').value)||0);if(!count)return;const p=familyPlayers.find(x=>x.id===playerId),fee=playerMonthlyFee(p);let cursor=latestKnownMonthForPlayer(playerId);for(let i=1;i<=count;i++){const month=addMonths(cursor,i),existing=familyCharges.find(c=>c.playerId===playerId&&c.month===month);if(existing&&['paid','exempt'].includes(existing.status))continue;if(existing&&activeReportForCharge(existing.id))continue;out.push({chargeId:existing?.id||'',playerId,playerCode:p?.playerCode||'',categoryId:p?.categoryId||'',month,amount:existing?chargeRemaining(existing):fee,createCharge:!existing})}});return out}
function currentBulkAllocations(){const existing=selectedBulkChargeIds().map(id=>familyCharges.find(c=>c.id===id)).filter(Boolean).map(c=>({chargeId:c.id,playerId:c.playerId,playerCode:c.playerCode||'',categoryId:c.categoryId||familyPlayers.find(p=>p.id===c.playerId)?.categoryId||'',month:c.month,amount:chargeRemaining(c),createCharge:false}));return[...existing,...futurePrepayAllocations()]}
function updateBulkPaymentTotal(){const allocations=currentBulkAllocations(),total=allocations.reduce((a,x)=>a+Number(x.amount||0),0);if($('#sinpeCalculatedTotal'))$('#sinpeCalculatedTotal').textContent=money(total);if($('#sinpeAmount'))$('#sinpeAmount').value=total||'';if($('#sinpeSelectionSummary'))$('#sinpeSelectionSummary').textContent=allocations.length?`${allocations.length} mensualidad${allocations.length===1?'':'es'} seleccionada${allocations.length===1?'':'s'}`:'Selecciona al menos una mensualidad';$('#sinpeSubmitButton').disabled=!allocations.length}
function renderBulkPaymentOptions(){const available=availableSinpeCharges().sort((a,b)=>String(a.month).localeCompare(String(b.month))||playerName(a.playerId).localeCompare(playerName(b.playerId)));$('#sinpeChargeList').innerHTML=available.map(c=>`<label class="bulk-charge-option"><input type="checkbox" value="${c.id}"><span><strong>${esc(playerName(c.playerId))}</strong><small>${esc(monthLabel(c.month))} · Saldo ${money(chargeRemaining(c))}${chargeDueState(c)==='overdue'?' · Morosa':''}</small></span><strong>${money(chargeRemaining(c))}</strong></label>`).join('')||'<p class="muted">No hay mensualidades pendientes existentes.</p>';$('#sinpePrepayPlayers').innerHTML=familyPlayers.filter(p=>playerMonthlyFee(p)>0).map(p=>`<div class="sinpe-prepay-row" data-player-id="${p.id}"><div><strong>${esc(p.name)}</strong><small>Mensualidad ${money(playerMonthlyFee(p))} · después de ${esc(monthLabel(latestKnownMonthForPlayer(p.id)))}</small></div><label>Meses adicionales<input type="number" min="0" max="24" value="0"></label></div>`).join('');updateBulkPaymentTotal()}
async function uploadPaymentReceipt(file){if(!file)return{receiptUrl:'',receiptPath:''};if(file.size>5*1024*1024)throw Error('El comprobante no puede superar 5 MB.');const blob=await compressImageFile(file,{maxDimension:1800}),id=`${Date.now()}-${Math.random().toString(36).slice(2,8)}`,path=`orgs/${ORG_ID}/paymentReceipts/${auth.currentUser.uid}/${id}.jpg`,rf=storageRef(storage,path);await uploadBytes(rf,blob,{contentType:'image/jpeg'});return{receiptUrl:await getDownloadURL(rf),receiptPath:path}}
$('#openSinpeButton').onclick=()=>{if(!availableSinpeCharges().length&&!familyPlayers.some(p=>playerMonthlyFee(p)>0))return alert('No hay mensualidades disponibles para reportar.');$('#sinpeForm').reset();$('#paymentMethod').value='sinpe';$('#sinpeDate').value=today();$('#sinpePhone').value=profile.phone||'';$('#sinpePayer').value=profile.fullName||'';renderBulkPaymentOptions();$('#sinpeDialog').showModal()};
document.addEventListener('change',e=>{if(e.target.closest('#sinpeChargeList')||e.target.closest('#sinpePrepayPlayers'))updateBulkPaymentTotal()});
$('#sinpeForm').onsubmit=async e=>{e.preventDefault();const b=$('#sinpeSubmitButton');try{const allocations=currentBulkAllocations();if(!allocations.length)throw Error('Selecciona al menos una mensualidad o agrega meses anticipados.');for(const a of allocations){if(a.chargeId&&activeReportForCharge(a.chargeId))throw Error(`Ya existe un pago reportado para ${playerName(a.playerId)} · ${monthLabel(a.month)}.`)}const calculated=allocations.reduce((z,a)=>z+Number(a.amount||0),0),amount=Number($('#sinpeAmount').value);if(amount<=0)throw Error('Ingresa el monto depositado.');const credit=Math.max(0,amount-calculated),shortfall=Math.max(0,calculated-amount);if(shortfall>0)throw Error(`El monto depositado es ${money(shortfall)} menor que las mensualidades seleccionadas.`);busy(b,true,'Enviando…');const receipt=await uploadPaymentReceipt($('#sinpeReceipt')?.files?.[0]);const first=allocations[0];await addDoc(collection(db,'sinpeReports'),{orgId:ORG_ID,userId:auth.currentUser.uid,chargeId:first.chargeId||'',playerId:first.playerId,playerCode:first.playerCode,month:first.month,paymentMethod:$('#paymentMethod').value,amount,date:$('#sinpeDate').value,time:$('#sinpeTime').value,bank:$('#sinpeBank').value.trim(),phone:$('#sinpePhone').value.trim(),payerName:$('#sinpePayer').value.trim(),reference:$('#sinpeReference').value.trim(),notes:$('#sinpeNotes').value.trim(),allocations,calculatedAmount:calculated,creditBalance:credit,...receipt,status:'reported',createdAt:serverTimestamp(),updatedAt:serverTimestamp()});$('#sinpeDialog').close();await loadFamilyData();toast(`Pago reportado: ${allocations.length} mensualidades${credit?` · saldo a favor ${money(credit)}`:''}.`)}catch(x){alert(err(x))}finally{busy(b,false)}};
