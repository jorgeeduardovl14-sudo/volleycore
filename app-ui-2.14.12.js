...
function syncPilotSettingsControls(){
  ...
  $('#pilotSurveyAllCategories').checked=(pilotSettings.surveyCategoryIds||[]).length===0;
  $('#pilotFeatureAllCategories').checked=(pilotSettings.featureCategoryIds||[]).length===0;
  ...
  $('#pilotSurveyCategorySettings').classList.toggle('pilot-settings-disabled',$('#pilotSurveyAllCategories').checked);
  $('#pilotFeatureCategorySettings').classList.toggle('pilot-settings-disabled',$('#pilotFeatureAllCategories').checked);
}
function renderPilotAdmin(){
  ...
  $('#pilotAdminSummary').innerHTML=[
    [pilotFeedback.length,'Evaluaciones'],
    [avgSat==='—'?'—':`${avgSat}/5`,'Satisfacción promedio'],
    [featureRequests.length,'Feature requests'],
    [featureRequests.filter(r=>(r.status||'new')==='new').length,'Nuevas por revisar']
...
$('#pilotSurveyAllCategories').onchange=()=>$('#pilotSurveyCategorySettings').classList.toggle('pilot-settings-disabled',$('#pilotSurveyAllCategories').checked);
$('#pilotFeatureAllCategories').onchange=()=>$('#pilotFeatureCategorySettings').classList.toggle('pilot-settings-disabled',$('#pilotFeatureAllCategories').checked);
$('#savePilotSettings').onclick=async()=>{
  if(profile?.role!=='admin')return;
  const surveyAll=$('#pilotSurveyAllCategories').checked,featureAll=$('#pilotFeatureAllCategories').checked;
