...
export default async (request) => {
  ...
  try {
    ...
    const [charge,user] = await Promise.all([
      getDocument("charges",chargeId),
      getDocument("users",decoded.uid),
    ]);
    ...
    const total=Number(charge.amount||0), paid=Number(charge.paidAmount||0);
    ...
    if (!onvoRes.ok) {
      ...
      return json(502,{error:onvo.message||onvo.error?.message||`ONVO rejected checkout (${onvoRes.status}).`});
    }
    ...
    const writes=[];
    if (sessionId) {
      writes.push(updateWrite("onvoCheckoutSessions",sessionId,{
        orgId:charge.orgId||"asbavol",chargeId,playerId:charge.playerId||"",userId:decoded.uid,
        month:charge.month||"",amount:remainingCRC,amountMinor:unitAmount,currency:"CRC",
        mode:secretKey.startsWith("onvo_test_")?"test":"live",status:"checkout_created",url:onvo.url
      },null,{exists:false}));
      writes.push(transformServerTimestampWrite("onvoCheckoutSessions",sessionId,"createdAt"));
    }
    writes.push(updateWrite("charges",chargeId,{
      onvoPaymentStatus:"checkout_created",
      onvoLastCheckoutSessionId:sessionId||null
    },["onvoPaymentStatus","onvoLastCheckoutSessionId"]));
