...
const PILOT_VERSION='UI 2.14.21';
...
function openEventDetail(id){
  ...
  $('#editEventFromDetail').classList.toggle('hidden',!(['admin','treasurer'].includes(profile?.role)||(isCoachingRole(profile?.role)&&sportsCategoryIds(e).every(id=>(profile.assignedCategoryIds||[]).includes(id)))));
...
$('#sportsDayDetailContent').onclick=e=>{const b=e.target.closest('[data-edit-event]');if(!b)return;$('#sportsDayDetailDialog').close();openEvent(events.find(x=>x.id===b.dataset.editEvent));};$('#newAnnouncementButton').onclick=()=>openAnnouncement();
