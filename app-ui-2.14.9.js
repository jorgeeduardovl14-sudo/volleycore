...
const viewGroups={
  admin:new Set(['dashboard','players','categories','seasons','trainings','attendance','events','venues','announcements','finances','sinpeAdmin','users','userAdmin','linkAdmin','offboarding','shirtAdmin','trainers','import','reports','profile','about']),
...
$('#offboardingReasonFilter').onchange=renderOffboarding;
...
function renderOffboarding(){
  ...
  const q=norm($('#offboardingSearch')?.value||''),type=$('#offboardingEntityFilter')?.value||'',reason=$('#offboardingReasonFilter')?.value||'';
  const list=exitRecords.filter(r=>(!type||r.entityType===type)&&(!reason||r.reason===reason)&&(!q||norm([r.entityName,r.destinationTeam,r.details,r.notes,offboardingReasonLabel(r.reason)].join(' ')).includes(q)));
  $('#offboardingSummary').innerHTML=[[exitRecords.length,'Bajas registradas'],[exitRecords.filter(r=>r.entityType==='player').length,'Jugadoras'],[exitRecords.filter(r=>r.entityType==='trainer').length,'Personal deportivo'],[exitRecords.filter(r=>['sport_dissatisfaction','administrative_dissatisfaction'].includes(r.reason)).length,'Por insatisfacción']].map(([v,l])=>`<article class="stat"><strong>${v}</strong><span>${l}</span></article>`).join('');
  $('#offboardingList').innerHTML=list.map(r=>`<article class="panel offboarding-card"><div class="page-head compact-head"><div><span class="eyebrow">${esc(offboardingTypeLabel(r.entityType))}</span><h3>${esc(r.entityName||'Registro')}</h3></div><span class="badge inactive">${r.exitType==='temporary'?'Temporal':'Definitiva'}</span></div><div class="grid two"><p><strong>Fecha efectiva:</strong> ${esc(r.effectiveDate||'—')}<br><strong>Motivo:</strong> ${esc(offboardingReasonLabel(r.reason))}${r.destinationTeam?`<br><strong>Equipo destino:</strong> ${esc(r.destinationTeam)}`:''}</p><p><strong>Procesado por:</strong> ${esc(r.processedByName||userName(r.processedBy)||'—')}${r.dissatisfactionArea?`<br><strong>Área de insatisfacción:</strong> ${esc(OFFBOARDING_DISSATISFACTION[r.dissatisfactionArea]||r.dissatisfactionArea)}`:''}</p></div><p>${esc(r.details||'')}</p>${r.notes?`<p class="muted">${esc(r.notes)}</p>`:''}</article>`).join('')||'<p class="muted">No hay bajas registradas para los filtros seleccionados.</p>';
}
...
document.addEventListener('keydown',e=>{
  ...
  const target=e.target.closest('[data-dashboard-view],[data-manage-role]');
