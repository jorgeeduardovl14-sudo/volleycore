...
let profile=null,players=[],categories=[],venues=[],events=[],announcements=[],charges=[],sinpeReports=[],onvoPayments=[],allUsers=[],familyUsers=[],trainerUsers=[],families=[],seasons=[],trainingSeries=[],trainingExceptions=[],linkRequests=[],categoryVideos=[],attendanceRecords=[],notifications=[];
const state={authStatus:'loading',currentView:null,adminListeners:[],familyListeners:[],familiesReconciled:false,familyDataLoading:false,familyPickerId:null,notificationUnsub:null};
...
function showApp(user,p){
  ...
  if(admin){go('dashboard');loadAdminData().then(()=>{startAdminRealtime();startNotificationRealtime();if(!state.familiesReconciled&&families.length===0&&players.some(x=>(x.linkedUserIds||[]).length)){state.familiesReconciled=true;reconcileFamilies(false).catch(console.error)}}).catch(e=>alert(err(e)))}
  else if(coachingStaff){go('trainerHome');loadTrainerData().then(()=>startNotificationRealtime()).catch(e=>alert(err(e)));}
  ...
  else{state.familyDataLoading=true;go('familyHome');loadFamilyData().then(()=>{startFamilyRealtime();startNotificationRealtime();handleOnvoReturn();}).catch(e=>alert(err(e)));const playerMode=isPlayerProfile(p);const payBtn=$('[data-view="familyPayments"]');if(payBtn)payBtn.classList.toggle('hidden',playerMode);const linkBtn=$('[data-view="familyLink"]');if(linkBtn)linkBtn.classList.toggle('hidden',playerMode);}
  if(admin||coachingStaff)setTimeout(handleOnvoReturn,0);
}

function showOut(){clearTimeout(bootWatchdog);state.authStatus='unauthenticated';stopRealtime();stopNotificationRealtime();$('#bootScreen').classList.add('hidden');$('#appScreen').classList.add('hidden');$('#authScreen').classList.remove('hidden')}
