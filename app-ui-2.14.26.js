...
function categoryDetailHTML(id,{familyMode=false}={}){
  ...
  ${categoryNumbersHTML(categoryId)}
  ${categoryGalleryHTML(categoryId)}
...
function renderFinanceEnhancements(){const month=$('#financeInsightsMonth')?.value||monthNow(),cat=$('#financeInsightsCategory')?.value||'';if($('#financeInsightsMonth')&&!$('#financeInsightsMonth').value)$('#financeInsightsMonth').value=month;['#fixedExpenseCategory','#sponsorCategory'].forEach(sel=>{const el=$(sel);if(el){const old=el.value;el.innerHTML='<option value="">General / todas</option>'+categories.map(c=>`<option value="${c.id}">${esc(c.name)}</option>`).join('');el.value=old}});if($('#financeInsightsCategory')){const old=$('#financeInsightsCategory').value;$('#financeInsightsCategory').innerHTML='<option value="">Todas las categorías</option>'+categories.map(c=>`<option value="${c.id}">${esc(c.name)}</option>`).join('');$('#financeInsightsCategory').value=old}const incomes=categoryIncomeRows().filter(x=>(!month||String(x.date||'').slice(0,7)===month)&&(!cat||x.categoryId===cat)),expenses=fixedExpenses.filter(x=>x.active!==false&&(!cat||!x.categoryId||x.categoryId===cat)),incomeTotal=incomes.reduce((a,x)=>a+Number(x.amount||0),0),expenseTotal=expenses.reduce((a,x)=>a+Number(x.amount||0),0);if($('#financeInsightsSummary'))$('#financeInsightsSummary').innerHTML=[[money(incomeTotal),'Ingreso real'],[money(expenseTotal),'Gastos fijos'],[money(incomeTotal-expenseTotal),'Resultado estimado'],[incomes.filter(x=>x.kind==='sponsor').length,'Patrocinios']].map(([v,l])=>`<article class="stat"><strong>${v}</strong><span>${l}</span></article>`).join('');if($('#categoryIncomeBody')){const by=new Map();categories.forEach(c=>by.set(c.id,{name:c.name,payments:0,sponsors:0}));incomes.forEach(x=>{if(!x.categoryId)return;const z=by.get(x.categoryId)||{name:catName(x.categoryId),payments:0,sponsors:0};z[x.kind==='sponsor'?'sponsors':'payments']+=Number(x.amount||0);by.set(x.categoryId,z)});$('#categoryIncomeBody').innerHTML=[...by.entries()].filter(([id])=>!cat||id===cat).map(([id,z])=>`<tr><td><strong>${esc(z.name)}</strong></td><td>${money(z.payments)}</td><td>${money(z.sponsors)}</td><td><strong>${money(z.payments+z.sponsors)}</strong></td></tr>`).join('')}if($('#fixedExpensesList'))$('#fixedExpensesList').innerHTML=expenses.map(x=>`<article class="panel"><div class="page-head compact-head"><div><span class="eyebrow">${esc(x.categoryId?catName(x.categoryId):'General')} · Mensual</span><h4>${esc(x.name||'Gasto fijo')}</h4></div><strong>${money(x.amount)}</strong></div><p class="muted">Día de pago: ${esc(String(x.dueDay||'—'))} · ${esc(x.notes||'')}</p></article>`).join('')||'<p class="muted">No hay gastos fijos registrados.</p>';if($('#sponsorIncomeList'))$('#sponsorIncomeList').innerHTML=sponsorIncome.sort((a,b)=>String(b.date||'').localeCompare(String(a.date||''))).map(x=>`<article class="panel"><div class="page-head compact-head"><div><span class="eyebrow">${esc(x.categoryId?catName(x.categoryId):'General')} · ${esc(x.date||'')}</span><h4>${esc(x.sponsorName||'Patrocinador')}</h4></div><strong>${money(x.amount)}</strong></div><p class="muted">${esc(x.concept||'Patrocinio')} ${x.reference?'· Ref. '+esc(x.reference):''}</p></article>`).join('')||'<p class="muted">No hay ingresos de patrocinadores.</p>'}
let bankImportRows=[];
...
function reconcileBankRows(){const app=approvedPaymentRows(),used=new Set();return bankImportRows.map(b=>{let best=null,score=0;app.forEach(a=>{if(used.has(a.id))return;let z=0;if(b.reference&&a.reference&&norm(b.reference)===norm(a.reference))z+=100;if(Math.abs(b.amount-a.amount)<.01)z+=30;if(b.date&&a.date&&b.date===a.date)z+=20;if(z>score){score=z;best=a}});if(best&&score>=30)used.add(best.id);return{...b,match:score>=30?best:null}}).concat(app.filter(a=>!used.has(a.id)).map(a=>({id:'app-'+a.id,date:a.date,description:a.description,reference:a.reference,amount:a.amount,appOnly:a})))}
...
async function loadCommunityData(){try{[categoryGallery,fixedExpenses,sponsorIncome]=await Promise.all([col('categoryGallery',[where('orgId','==',ORG_ID)]),col('fixedExpenses',[where('orgId','==',ORG_ID)]),col('sponsorIncome',[where('orgId','==',ORG_ID)])])}catch(e){console.error('Community/finance data load error',e)}}
...
$('#financeInsightsMonth')?.addEventListener('change',renderFinanceEnhancements);$('#financeInsightsCategory')?.addEventListener('change',renderFinanceEnhancements);
