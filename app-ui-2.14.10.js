...
function renderCharges(){
  ...
  $('#financeSummary').innerHTML=[
    ...
    [list.length,'Cargos','']
  ].map(([v,l,s])=>`<button type="button" class="stat clickable-stat finance-summary-card" data-finance-status="${s}"><strong>${v}</strong><span>${l}</span></button>`).join('');
...
document.addEventListener('click',e=>{
  ...
  const off=e.target.closest('[data-offboarding-detail]');if(off){openOffboardingDetail(off.dataset.offboardingDetail);return;}
...
$('#financeSummary').onclick=e=>{
  const card=e.target.closest('[data-finance-status]');if(!card)return;
  $('#chargeStatusFilter').value=card.dataset.financeStatus||'';
  renderCharges();
