...
let profile=null,playerPrivateById=new Map(),players=[],categories=[],venues=[],events=[],announcements=[],charges=[],sinpeReports=[],onvoPayments=[],shirtRequests=[],allUsers=[],familyUsers=[],trainerUsers=[],families=[],seasons=[],trainingSeries=[],trainingExceptions=[],linkRequests=[],categoryVideos=[],attendanceRecords=[],exitRecords=[],notifications=[];
...
const viewGroups={
  admin:new Set(['dashboard','players','categories','seasons','trainings','attendance','events','venues','announcements','finances','sinpeAdmin','users','userAdmin','linkAdmin','exitLog','shirtAdmin','trainers','import','reports','profile','about']),
  coaching:new Set(['trainerHome','trainerPlayers','trainerTrainings','trainerAttendance','trainerEvents','trainerAnnouncements','profile','about']),
  family:new Set(['familyHome','familyCategories','familyLink','familyTrainings','familyEvents','familyPayments','familyAnnouncements','profile','about']),
...
function renderShirtAdmin(){
  ...
  $('#shirtAdminSummary').innerHTML=[
    ...
    [shirtRequests.filter(r=>r.status==='pending').length,'Pendientes'],
    [shirtRequests.filter(r=>r.status==='processing').length,'En proceso'],
    [shirtRequests.filter(r=>r.status==='ready').length,'Listas para entregar']
  ].map(([v,l])=>`<article class="stat"><strong>${v}</strong><span>${l}</span></article>`).join('');
  $('#shirtAdminList').innerHTML=list.map(r=>`<article class="panel shirt-admin-card">
...
function userRoleLabel(uOrRole){
  ...
  return role==='admin'?'Administrador':role==='treasurer'?'Tesorería':role==='trainer'?'Entrenador':role==='assistant'?'Asistente':role==='player'?'Jugadora':role==='pendingTrainer'?'Entrenador pendiente':role==='pendingAssistant'?'Asistente pendiente':'Familia';
}
...
function openRoleManager(u){
  ...
  $('#deleteUserFromRoleManager').classList.toggle('hidden',u.id===auth.currentUser?.uid);
...
$('#exitRecordReason').onchange=syncExitRecordConditionalFields;
$('#exitRecordCancel').onclick=()=>finishExitRecord(null);
$('#exitRecordClose').onclick=()=>finishExitRecord(null);
$('#exitRecordDialog').oncancel=e=>{e.preventDefault();finishExitRecord(null);};
$('#exitRecordForm').onsubmit=e=>{
  e.preventDefault();
  const reason=$('#exitRecordReason').value;
  if(!reason)return alert('Selecciona el motivo principal de la baja.');
  if(reason==='transfer'&&!$('#exitRecordDestination').value.trim())return alert('Indica a cuál equipo u organización se trasladó.');
  if(reason==='dissatisfaction'&&!$('#exitRecordDissatisfaction').value)return alert('Selecciona el área principal de insatisfacción.');
  finishExitRecord(exitRecordDataFromForm());
};

$('#exitLogSearch').oninput=renderExitLog;
$('#exitLogEntityType').onchange=renderExitLog;
$('#exitLogReason').onchange=renderExitLog;
$('#exitLogCategory').onchange=renderExitLog;
$('#exitLogMonth').onchange=renderExitLog;
...
$('#roleManagerForm').onsubmit=async e=>{
  ...
  let exitData=null;
  if(u.status!=='inactive'&&status==='inactive'){
    const isCoach=['trainer','assistant','pendingTrainer','pendingAssistant'].includes(u.role)||['trainer','assistant'].includes(newRole);
    exitData=await requestExitRecord({
      entityType:isCoach?'trainer':'user',
      entityId:uid,
      entityName:u.fullName||u.email,
      context:`${userRoleLabel(effectiveUserRole(u))}${u.email?' · '+u.email:''}`,
      categoryIds:u.assignedCategoryIds||[],
      relatedRole:newRole
    });
    if(!exitData)return;
  }
  ...
  const oldRole=effectiveUserRole(u);
  if(!confirm(`¿Cambiar el rol de ${u.fullName||u.email} de ${userRoleLabel(oldRole)} a ${userRoleLabel(newRole)}?`))return;
  ...
  if(exitData){const x=exitRecordRefWithData(exitData);batch.set(x.ref,x.data);}
...
$('#trainerForm').onsubmit=async e=>{
  ...
  const current=allUsers.find(x=>x.id===uid),nextStatus=$('#trainerStatus').value;
  let exitData=null;
  if(current?.status!=='inactive'&&nextStatus==='inactive'){
    exitData=await requestExitRecord({entityType:'trainer',entityId:uid,entityName:current?.fullName||current?.email||'Entrenador',context:selectedRole==='assistant'?'Asistente':'Entrenador',categoryIds:assignedCategoryIds,relatedRole:selectedRole});
    if(!exitData)return;
  }
  const batch=writeBatch(db);
...
const exitReasonLabel=v=>({transfer:'Cambio de equipo',dissatisfaction:'Insatisfacción',schedule:'Incompatibilidad de horarios',financial:'Razones económicas',injury:'Lesión / salud',study:'Estudio',move:'Mudanza',family:'Decisión familiar',retirement:'Retiro del deporte',administrative:'Razón administrativa',other:'Otro'}[v]||v||'—');
const exitDissatisfactionLabel=v=>({coach:'Entrenador / cuerpo técnico',playing_time:'Tiempo de juego / rol deportivo',communication:'Comunicación',treatment:'Trato recibido',organization:'Organización',facilities:'Instalaciones',cost:'Costos / pagos',schedule:'Horarios',team_environment:'Ambiente de equipo',other:'Otro'}[v]||v||'—');
const exitEntityTypeLabel=v=>({player:'Jugadora',user:'Usuario',trainer:'Entrenador / asistente'}[v]||v||'—');
const exitTypeLabel=v=>v==='temporary'?'Temporal':'Definitiva';
const exitReturnLabel=v=>({yes:'Sí',no:'No',unknown:'No determinado'}[v]||'No determinado');

let exitRecordResolver=null;
function syncExitRecordConditionalFields(){
  const reason=$('#exitRecordReason')?.value||'';
  $('#exitDestinationField')?.classList.toggle('hidden',reason!=='transfer');
  $('#exitDissatisfactionField')?.classList.toggle('hidden',reason!=='dissatisfaction');
}
function requestExitRecord({entityType,entityId,entityName,context='',categoryIds=[],relatedRole=''}) {
  if(profile?.role!=='admin')return Promise.resolve(null);
  const d=$('#exitRecordDialog'),form=$('#exitRecordForm');
  form.reset();
  $('#exitRecordEntityType').value=entityType;
  $('#exitRecordEntityId').value=entityId;
  $('#exitRecordEntityName').textContent=entityName||'Persona';
  $('#exitRecordEntityContext').textContent=context||exitEntityTypeLabel(entityType);
  $('#exitRecordDate').value=today();
  $('#exitRecordExitType').value='definitive';
  $('#exitRecordReturn').value='unknown';
  d.dataset.categoryIds=JSON.stringify(categoryIds||[]);
  d.dataset.relatedRole=relatedRole||'';
  syncExitRecordConditionalFields();
  if(d.open)d.close();
  d.showModal();
  return new Promise(resolve=>{exitRecordResolver=resolve;});
}
function finishExitRecord(value){
  if(!exitRecordResolver)return;
  const resolve=exitRecordResolver;exitRecordResolver=null;
  if($('#exitRecordDialog')?.open)$('#exitRecordDialog').close();
  resolve(value);
}
function exitRecordDataFromForm(){
  const d=$('#exitRecordDialog'),reason=$('#exitRecordReason').value;
  return {
    orgId:ORG_ID,
    entityType:$('#exitRecordEntityType').value,
    entityId:$('#exitRecordEntityId').value,
    entityName:$('#exitRecordEntityName').textContent.trim(),
    relatedRole:d.dataset.relatedRole||'',
    categoryIds:JSON.parse(d.dataset.categoryIds||'[]'),
    exitDate:$('#exitRecordDate').value,
    exitType:$('#exitRecordExitType').value,
    reason,
    destinationTeam:reason==='transfer'?$('#exitRecordDestination').value.trim():'',
    dissatisfactionArea:reason==='dissatisfaction'?$('#exitRecordDissatisfaction').value:'',
    returnPossible:$('#exitRecordReturn').value,
    detail:$('#exitRecordDetail').value.trim(),
    notes:$('#exitRecordNotes').value.trim(),
    processedBy:auth.currentUser.uid,
    processedByName:profile?.fullName||auth.currentUser.email||'Administrador',
    createdAt:serverTimestamp(),
    updatedAt:serverTimestamp()
  };
}
function exitRecordRefWithData(data){
  const ref=doc(collection(db,'exitRecords'));
  return {ref,data:{...data,idHint:ref.id}};
}
function populateExitLogCategories(){
  const el=$('#exitLogCategory');if(!el)return;
  const current=el.value;
  el.innerHTML='<option value="">Todas las categorías</option>'+categories.map(c=>`<option value="${c.id}">${esc(c.name)}</option>`).join('');
  if([...el.options].some(o=>o.value===current))el.value=current;
}
function renderExitLog(){
  if(!$('#exitLogList'))return;
  populateExitLogCategories();
  const q=norm($('#exitLogSearch')?.value||''),type=$('#exitLogEntityType')?.value||'',reason=$('#exitLogReason')?.value||'',cat=$('#exitLogCategory')?.value||'',month=$('#exitLogMonth')?.value||'';
  const list=exitRecords.filter(r=>{
    ...
    if(month&&!(r.exitDate||'').startsWith(month))return false;
    if(q&&!norm([r.entityName,r.destinationTeam,r.detail,r.notes,r.processedByName,exitReasonLabel(r.reason)].join(' ')).includes(q))return false;
    return true;
  });
  const thisYear=String(new Date().getFullYear());
  $('#exitLogSummary').innerHTML=[
    [exitRecords.filter(r=>(r.exitDate||'').startsWith(thisYear)).length,'Bajas este año'],
    [exitRecords.filter(r=>r.entityType==='player').length,'Jugadoras'],
    [exitRecords.filter(r=>r.reason==='transfer').length,'Cambios de equipo'],
    [exitRecords.filter(r=>r.reason==='dissatisfaction').length,'Por insatisfacción']
  ].map(([v,l])=>`<article class="stat"><strong>${v}</strong><span>${l}</span></article>`).join('');
  $('#exitLogList').innerHTML=list.map(r=>`<article class="panel exit-log-card">
    <div class="page-head compact-head">
      <div><span class="eyebrow">${esc(exitEntityTypeLabel(r.entityType))} · ${esc(r.exitDate||'—')}</span><h3>${esc(r.entityName||'—')}</h3></div>
      <span class="badge inactive">${esc(exitTypeLabel(r.exitType))}</span>
    </div>
    <div class="grid two">
      <p><strong>Motivo:</strong> ${esc(exitReasonLabel(r.reason))}${r.reason==='dissatisfaction'&&r.dissatisfactionArea?`<br><strong>Área:</strong> ${esc(exitDissatisfactionLabel(r.dissatisfactionArea))}`:''}${r.destinationTeam?`<br><strong>Destino:</strong> ${esc(r.destinationTeam)}`:''}</p>
      <p><strong>Posible retorno:</strong> ${esc(exitReturnLabel(r.returnPossible))}<br><strong>Procesó:</strong> ${esc(r.processedByName||'—')}</p>
    </div>
    ${r.detail?`<p><strong>Detalle:</strong> ${esc(r.detail)}</p>`:''}
    ${r.notes?`<p class="muted"><strong>Observaciones:</strong> ${esc(r.notes)}</p>`:''}
  </article>`).join('')||'<p class="muted">No hay bajas para los filtros seleccionados.</p>';
}
function renderUserAdmin(){
  ...
  $('#userAdminSummary').innerHTML=[[allUsers.length,'Usuarios'],[allUsers.filter(u=>effectiveUserRole(u)==='player').length,'Jugadoras'],[allUsers.filter(u=>['trainer','assistant'].includes(effectiveUserRole(u))).length,'Personal deportivo'],[allUsers.filter(u=>(u.status||'active')==='active').length,'Activos']].map(([v,l])=>`<article class="stat"><strong>${v}</strong><span>${l}</span></article>`).join('');
  $('#userAdminBody').innerHTML=list.map(u=>{
    ...
    return `<tr><td><strong>${esc(u.fullName||'—')}</strong><div class="muted">${esc(u.accountType||'')}</div></td><td>${esc(u.email||'')}</td><td>${esc(u.phone||'')}</td><td><span class="badge neutral">${esc(userRoleLabel(u))}</span></td><td>${playerCell}</td><td><span class="badge ${u.status||'active'}">${esc(statusLabel(u.status||'active'))}</span></td><td><button class="btn secondary" data-manage-role="${u.id}" type="button">Gestionar rol</button></td></tr>`;
...
function renderFamilyHome(){
  ...
  $('#familyUpcoming').innerHTML=familyEvents.filter(e=>e.date>=today()&&e.status!=='cancelled').slice(0,5).map(eventMini).join('')||'<p class="muted">No hay eventos próximos.</p>';
...
$('#playerForm').onsubmit=async e=>{
  ...
  try{
    ...
    const existingPlayer=id?players.find(p=>p.id===id):null,nextPlayerStatus=$('#playerStatus').value;
    let exitData=null;
    if(existingPlayer&&existingPlayer.status!=='inactive'&&nextPlayerStatus==='inactive'){
      exitData=await requestExitRecord({entityType:'player',entityId:id,entityName:$('#playerName').value.trim(),context:`${$('#playerCode').value.trim()} · ${catName(primaryCategoryId)}`,categoryIds,relatedRole:'player'});
      if(!exitData)return;
    }
    const data={orgId:ORG_ID,playerCode:$('#playerCode').value.trim(),name:$('#playerName').value.trim(),birthdate:$('#playerBirthdate').value,nationalTeam:$('#playerNationalTeam').checked,categoryId:primaryCategoryId,reinforcementCategoryIds,categoryIds,permanentReinforcement:$('#playerPermanentReinforcement').checked,number:$('#playerNumber').value.trim(),position:$('#playerPosition').value.trim(),customFee:isFirstDivisionCategoryId(primaryCategoryId)?null:(feeRaw===''?null:Number(feeRaw)||0),status:nextPlayerStatus,notes:$('#playerNotes').value.trim(),linkedUserIds,updatedAt:serverTimestamp()};
    ...
    if(id){
      const batch=writeBatch(db);
      batch.update(doc(db,'players',id),data);
      charges.filter(c=>c.playerId===id).forEach(c=>batch.update(doc(db,'charges',c.id),{userIds:linkedUserIds,categoryId:primaryCategoryId,updatedAt:serverTimestamp()}));
      if(exitData){const x=exitRecordRefWithData(exitData);batch.set(x.ref,x.data);}
      await batch.commit();
    }else{
      const ref=await addDoc(collection(db,'players'),{...data,createdAt:serverTimestamp()});playerId=ref.id;
    }
...
state.adminListeners.push(onSnapshot(org('exitRecords'),snap=>{exitRecords=snap.docs.map(d=>({id:d.id,...d.data()})).sort((a,b)=>(b.exitDate||'').localeCompare(a.exitDate||'')||(b.createdAt?.seconds||0)-(a.createdAt?.seconds||0));if(state.currentView==='exitLog')renderExitLog();},console.error));}
