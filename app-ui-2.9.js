...
function renderUsers(){
  ...
  $('#usersBody').innerHTML=allUsers.map(u=>`<tr><td><strong>${esc(u.fullName||'—')}</strong><div class="muted">${esc(u.accountType||'')}</div></td><td>${esc(u.email||'')}</td><td>${esc(u.phone||'')}</td><td><span class="badge neutral">${esc(userRoleLabel(u))}</span></td><td>${players.filter(p=>(p.linkedUserIds||[]).includes(u.id)).map(p=>`<button class="chip chip-button" data-view-player="${p.id}">${esc(p.name)}</button>`).join('')||'—'}</td><td><span class="badge ${u.status||'active'}">${esc(statusLabel(u.status||'active'))}</span></td><td>${profile?.role==='admin'?`<button class="action" data-manage-role="${u.id}">Gestionar rol</button>`:'—'}</td></tr>`).join('')||'<tr><td colspan="7">No hay usuarios registrados.</td></tr>';
