...
function financePeriod(){
  let from=$('#financePeriodFrom')?.value||monthNow(),to=$('#financePeriodTo')?.value||from;
...
function renderFinanceEnhancements(){
  const{from,to}=financePeriod(),cat=$('#financeGlobalCategory')?.value||'',months=financePeriodMonthCount();
  if($('#financePeriodFrom')&&!$('#financePeriodFrom').value)$('#financePeriodFrom').value=from;
  if($('#financePeriodTo')&&!$('#financePeriodTo').value)$('#financePeriodTo').value=to;
...
function renderCharges(){renderFinanceEnhancements();renderBankReconciliation();
  const {from:periodFrom,to:periodTo}=financePeriod(),cat=$('#financeGlobalCategory')?.value||$('#chargeCategoryFilter')?.value||'',st=$('#chargeStatusFilter').value;
...
['financePeriodFrom','financePeriodTo','financeGlobalCategory'].forEach(id=>$('#'+id)?.addEventListener('change',()=>{renderCharges();renderFinanceEnhancements();renderBankReconciliation()}));document.addEventListener('click',e=>{const b=e.target.closest('[data-finance-tab]');if(b)setFinanceTab(b.dataset.financeTab)});
