...
const PILOT_VERSION='UI 2.14.18';
...
function normalizeCategoryCalendarRow(row,index,categoryId){
  ...
  const errors=[];if(!date)errors.push('Fecha inválida');if(!startTime)errors.push('Hora de inicio requerida');if(type==='match'&&!homeAway)errors.push('Define Casa/Visita');if(homeAway==='home'&&venueText&&!venue)errors.push('Sede local no registrada');
  return{index:index+1,date,startTime,endTime,type,opponent,homeAway,venueText,venueId:homeAway==='home'?(venue?.id||''):'',awayVenueName:homeAway==='away'?venueText:'',awayAddress:homeAway==='away'?address:'',notes,duplicateId:duplicate?.id||'',status:errors.length?'invalid':duplicate?'duplicate':'valid',errors};
}
function renderCategoryCalendarPreview(){
  ...
  $('#categoryCalendarImportPreview').innerHTML=`<table><thead><tr><th>Fila</th><th>Fecha</th><th>Horario</th><th>Tipo</th><th>Rival</th><th>Casa/Visita</th><th>Sede</th><th>Estado</th></tr></thead><tbody>${categoryCalendarPreviewRows.map(r=>`<tr><td>${r.index}</td><td>${esc(r.date||'—')}</td><td>${esc(r.startTime||'—')}${r.endTime?' – '+esc(r.endTime):''}</td><td>${esc(typeLabel(r.type))}</td><td>${esc(r.opponent||'—')}</td><td>${esc(r.homeAway==='home'?'Casa':r.homeAway==='away'?'Visita':'—')}</td><td>${esc(r.venueText||'—')}</td><td><span class="badge ${r.status==='valid'?'active':r.status==='duplicate'?'pending':'inactive'}">${r.status==='valid'?'Lista':r.status==='duplicate'?'Duplicado · se omitirá':esc(r.errors.join(', '))}</span></td></tr>`).join('')}</tbody></table>`;
  $('#confirmCategoryCalendarButton').disabled=!valid;
}
async function previewCategoryCalendar(){
  const file=$('#categoryCalendarImportFile').files[0];if(!file)return alert('Selecciona un archivo Excel o CSV.');
  if(typeof XLSX==='undefined')return alert('No fue posible cargar el lector de Excel.');
  const data=await file.arrayBuffer(),wb=XLSX.read(data,{type:'array',cellDates:true}),sheet=wb.Sheets[wb.SheetNames[0]],rows=XLSX.utils.sheet_to_json(sheet,{defval:'',raw:true});
  const categoryId=$('#calendarImportCategoryId').value;categoryCalendarPreviewRows=rows.map((r,i)=>normalizeCategoryCalendarRow(r,i,categoryId));renderCategoryCalendarPreview();
}
