...
const PILOT_VERSION='UI 2.14.24';
...
function renderDashboard(){
  ...
  const upcomingEvents=events.filter(e=>e.date>=today()&&e.status!=='cancelled').sort((a,b)=>`${a.date} ${a.startTime||''}`.localeCompare(`${b.date} ${b.startTime||''}`)).slice(0,5);
  ...
  $('#dashboardNextMatch').innerHTML=upcomingEvents.length?`
    <div class="vc-dashboard-match-list">${upcomingEvents.map(e=>`<div class="vc-dashboard-match-row"><strong>${esc(sportsCategoryNames(e))}</strong><span>${e.type==='match'&&e.opponent?`vs ${esc(e.opponent)}`:esc(e.title||typeLabel(e.type))}</span><small>${dashboardFormatDate(e.date)} · ${esc(e.startTime||'Hora por definir')} · ${esc(eventLocationName(e))}</small></div>`).join('')}</div>
