...
import {getFirestore,doc,getDoc,setDoc,updateDoc,serverTimestamp,collection,getDocs,query,where,addDoc,writeBatch,onSnapshot} from 'https://www.gstatic.com/firebasejs/10.12.5/firebase-firestore.js';
...
let profile=null,players=[],categories=[],venues=[],events=[],announcements=[],charges=[],sinpeReports=[],familyUsers=[],families=[],seasons=[],trainingSeries=[],trainingExceptions=[],linkRequests=[];
...
function showApp(user,p){clearTimeout(bootWatchdog);profile=p;state.authStatus='authenticated';$('#bootScreen').classList.add('hidden');$('#authScreen').classList.add('hidden');$('#appScreen').classList.remove('hidden');const admin=['admin','treasurer'].includes(p.role);$('#adminNav').classList.toggle('hidden',!admin);$('#familyNav').classList.toggle('hidden',admin);$('#roleBadge').textContent=p.role==='admin'?'Administrador':p.role==='treasurer'?'Tesorería':p.accountType==='player'?'Jugadora':'Familia';$('#profileName').textContent=p.fullName||'—';$('#profileEmail').textContent=p.email||user.email;$('#profilePhone').textContent=p.phone||'—';$('#profileRole').textContent=p.role+(p.accountType?` · ${p.accountType==='player'?'jugadora':'encargado/a'}`:'');$('#editFullName').value=p.fullName||'';$('#editPhone').value=p.phone||'';$('#emailNotice').classList.toggle('hidden',user.emailVerified);stopRealtime();if(admin){go('dashboard');loadAdminData().then(()=>{startAdminRealtime();if(!state.familiesReconciled&&families.length===0&&players.some(x=>(x.linkedUserIds||[]).length)){state.familiesReconciled=true;reconcileFamilies(false).catch(console.error)}}).catch(e=>alert(err(e)))}else{state.familyDataLoading=true;go('familyHome');loadFamilyData().then(startFamilyRealtime).catch(e=>alert(err(e)));const payBtn=$('[data-view="familyPayments"]');if(payBtn)payBtn.classList.toggle('hidden',p.accountType==='player');}}
