...
export function safeOrigin(request, supplied) {
  ...
  for (const raw of [process.env.URL, process.env.DEPLOY_PRIME_URL]) {
    if (!raw) continue;
    try { allowed.add(new URL(raw).origin); } catch {}
  }
  for (const raw of [supplied, request.headers.get("origin"), process.env.URL]) {
    ...
    try {
      ...
      if (u.hostname.endsWith(".netlify.app") || allowed.has(u.origin)) return u.origin;
    } catch {}
  }
...
export async function getServiceAccessToken() {
  ...
  if (!res.ok || !data.access_token) throw new Error(data.error_description || data.error || "Could not obtain Google service access token.");
...
export async function verifyFirebaseUser(request) {
  ...
  if (u.disabled) throw new Error("Firebase user is disabled.");
...
function docUrl(collection,id) {
  return `https://firestore.googleapis.com/v1/${docName(collection,id)}`;
}
...
export function updateWrite(collection,id,fields,maskFields=null,precondition=null) {
  const write = {
    update:{name:docName(collection,id),fields:encodeFields(fields)},
  };
  const mask = maskFields || Object.keys(fields);
  if (mask?.length) write.updateMask={fieldPaths:mask};
  if (precondition) write.currentDocument=precondition;
  return write;
}
export function transformServerTimestampWrite(collection,id,field="updatedAt") {
  return {
    transform:{
      document:docName(collection,id),
      fieldTransforms:[{fieldPath:field,setToServerValue:"REQUEST_TIME"}]
    }
  };
}
...
export async function commitWrites(writes) {
  ...
  const data = await res.json().catch(()=>({}));
  if (!res.ok) {
    const error = new Error(data.error?.message || `Firestore commit failed (${res.status}).`);
