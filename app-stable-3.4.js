...
let profile=null,players=[],categories=[],venues=[],events=[],announcements=[],charges=[],sinpeReports=[],allUsers=[],familyUsers=[],trainerUsers=[],families=[],seasons=[],trainingSeries=[],trainingExceptions=[],linkRequests=[],categoryVideos=[],attendanceRecords=[];
...
function categoryDetailHTML(id,{familyMode=false}={}){
  ...
  return `<div class="category-detail-grid">
    <article class="panel">
...
function renderPlayers(){
  ...
  const list=players.filter(p=>(!cat||playerCatIds(p).includes(cat))&&(!st||p.status===st)&&(!q||norm([p.playerCode,p.name,p.phone,p.email,...(p.guardians||[]).flatMap(g=>[g.name,g.phone,g.phone2])].join(' ')).includes(q)));
...
function attendanceRowHTML(p,entry,prefix){
  const status=entry?.status||'present',reason=entry?.reason||'',note=entry?.note||'';
  return `<div class="attendance-row" data-player-id="${p.id}">
    <div><strong>${esc(p.name)}</strong><div class="muted">${esc(p.playerCode||'')} · ${p.categoryId===($('#'+prefix+'Category')?.value||'')?'Categoría principal':'Refuerzo'}</div></div>
    ...
    <div class="attendance-absence ${status==='absent'?'':'hidden'}">
      <select class="attendance-reason"><option value="">Motivo</option>${absenceReasons().map(x=>`<option value="${x}" ${x===reason?'selected':''}>${x}</option>`).join('')}</select>
      <input class="attendance-note" value="${esc(note)}" placeholder="Justificación / observación">
    </div>
  </div>`;
}
function bindAttendanceRoster(containerId,prefix){
  ...
  container.onchange=e=>{
    if(e.target.matches('input[type="radio"]')){
      const row=e.target.closest('.attendance-row'),box=row.querySelector('.attendance-absence');
      box.classList.toggle('hidden',e.target.value!=='absent');
    }
  };
}
...
async function saveAttendance(prefix,{trainer=false}={}){
  ...
  const entries=rows.map(row=>{
    const status=row.querySelector('input[type="radio"]:checked')?.value||'present';
    return {playerId:row.dataset.playerId,status,reason:status==='absent'?(row.querySelector('.attendance-reason')?.value||''):'',note:status==='absent'?(row.querySelector('.attendance-note')?.value.trim()||''):''};
  });
...
function attendanceSummaryRows(categoryId,month,sourcePlayers=players){
  ...
  return roster.map(p=>{
    ...
    return {player:p,total,present,absent,justified:entries.filter(e=>e.status==='absent'&&(e.reason||e.note)).length,percent:total?Math.round(present*100/total):0};
  });
}
function renderAttendanceSummary(prefix,{trainer=false}={}){
  ...
  $('#'+target).innerHTML=`<div class="table-wrap"><table><thead><tr><th>Jugadora</th><th>Registros</th><th>Presentes</th><th>Ausentes</th><th>Justificadas</th><th>% asistencia</th></tr></thead><tbody>${rows.map(r=>`<tr><td>${esc(r.player.name)}</td><td>${r.total}</td><td>${r.present}</td><td>${r.absent}</td><td>${r.justified}</td><td><strong>${r.percent}%</strong></td></tr>`).join('')||'<tr><td colspan="6">Sin registros.</td></tr>'}</tbody></table></div>`;
}
...
function exportAttendanceCSV(){
  ...
  downloadCSV(`asistencia-${catName(categoryId)}-${month}.csv`,rows.map(r=>({Jugadora:r.player.name,PlayerID:r.player.playerCode,Registros:r.total,Presentes:r.present,Ausentes:r.absent,Justificadas:r.justified,'Porcentaje asistencia':`${r.percent}%`})));
}
...
function openPlayer(p){
  $('#playerForm').reset();$('#playerDocId').value=p?.id||'';$('#playerCode').value=p?.playerCode||nextCode();$('#playerName').value=p?.name||'';$('#playerBirthdate').value=p?.birthdate||'';$('#playerType').value=p?.playerType||'minor';
  ...
  $('#guardiansEditor').innerHTML=(p?.guardians?.length?p.guardians:[{}]).map(guardianRow).join('');$('#linkedUsersEditor').innerHTML=linkedUsersHTML(p?.linkedUserIds||[]);
...
$('#playerForm').onsubmit=async e=>{
  ...
  try{
    ...
    if(id){await updateDoc(doc(db,'players',id),data);const related=charges.filter(c=>c.playerId===id);if(related.length){const batch=writeBatch(db);related.forEach(c=>batch.update(doc(db,'charges',c.id),{userIds:linkedUserIds,categoryId:primaryCategoryId,updatedAt:serverTimestamp()}));await batch.commit()}}else await addDoc(collection(db,'players'),{...data,createdAt:serverTimestamp()});
...
$('#categoryForm').onsubmit=async e=>{
  ...
  try{
    ...
    await loadAdminData();
    const saved=categories.find(c=>c.id===categoryId);
    openCategory(saved);
...
function openPlayerDetail(id){
  try{
    ...
    const adminAllowed=['admin','treasurer'].includes(profile.role),familyAllowed=(p.linkedUserIds||[]).includes(auth.currentUser.uid),trainerAllowed=profile.role==='trainer'&&playerCatIds(p).some(cid=>(profile.assignedCategoryIds||[]).includes(cid));
