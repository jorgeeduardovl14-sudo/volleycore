...
const PILOT_VERSION='UI 2.14.14';
...
let trainingWeekOffset=0,trainingViewMode='week';
...
function renderTrainings(){
 ...
 const cat=$('#trainingCategoryFilter').value,se=$('#trainingSeasonFilter').value,venue=$('#trainingVenueFilter').value;
 const list=trainingSeries.filter(t=>(!cat||t.categoryId===cat)&&(!se||t.seasonId===se)&&(!venue||t.venueId===venue)&&t.status!=='inactive');
 const ws=trainingWeekStart(),we=datePlus(ws,6),weekOcc=list.flatMap(t=>occurrences(t,ws,we).filter(o=>o.status!=='cancelled').map(o=>({...o,series:t})));
 ...
 $('#trainingStats').innerHTML=[[upcoming,'Próximos entrenamientos'],[weekOcc.length,'Esta semana'],[activeCats,'Categorías activas'],[hours.toFixed(hours%1?1:0),'Horas programadas']].map(([v,l])=>`<article class="stat"><strong>${v}</strong><span>${l}</span></article>`).join('');
 $('#trainingWeekLabel').textContent=`${ws} — ${we}`;
 const names=['Lunes','Martes','Miércoles','Jueves','Viernes','Sábado','Domingo'];
 $('#trainingCalendar').innerHTML=names.map((name,i)=>{const date=datePlus(ws,i),items=weekOcc.filter(o=>o.date===date).sort((a,b)=>(a.startTime||'').localeCompare(b.startTime||''));return`<section class="training-day"><header><strong>${name}</strong><span>${date.slice(8,10)}/${date.slice(5,7)}</span></header><div class="training-day-body">${items.map(o=>{const t=o.series;return`<button class="training-block" style="--training-color:${trainingColor(t.categoryId)}" data-edit-training="${t.id}" type="button"><span class="training-category-dot"></span><strong>${esc(catName(t.categoryId))}</strong><span>${esc(o.startTime)} – ${esc(o.endTime)}</span><small>${esc(venueName(o.venueId||t.venueId))}</small>${o.status==='changed'?'<em>Modificado</em>':''}</button>`}).join('')||'<span class="training-empty">Sin entrenamientos</span>'}</div></section>`}).join('');
 $('#trainingsList').innerHTML=list.map(t=>`<article class="panel training-list-card" style="--training-color:${trainingColor(t.categoryId)}"><div class="page-head compact-head"><div><span class="eyebrow">${esc(catName(t.categoryId))} · ${esc(seasonName(t.seasonId))}</span><h3>${esc(daysLabel(t.days))}</h3></div><span class="badge ${t.status}">${statusLabel(t.status)}</span></div><p><strong>${esc(t.startTime)} – ${esc(t.endTime)}</strong> · ${esc(venueName(t.venueId))}</p><div class="actions"><button class="action" data-edit-training="${t.id}">Editar serie</button><button class="action" data-training-exception="${t.id}">Cambiar/cancelar fecha</button><button class="action" data-training-future="${t.id}">Cambiar siguientes</button></div></article>`).join('')||'<p class="muted">No hay entrenamientos recurrentes.</p>';
 $('#trainingCalendar').classList.toggle('hidden',trainingViewMode!=='week');$('#trainingsList').classList.toggle('hidden',trainingViewMode!=='list');
}
...
$('#playerSearch').oninput=renderPlayers;$('#playerCategoryFilter').onchange=renderPlayers;$('#playerStatusFilter').onchange=renderPlayers;$('#eventCategoryFilter').onchange=renderEvents;$('#eventTypeFilter').onchange=renderEvents;$('#eventSeasonFilter').onchange=renderEvents;$('#trainingCategoryFilter').onchange=renderTrainings;$('#trainingSeasonFilter').onchange=renderTrainings;$('#trainingVenueFilter').onchange=renderTrainings;$('#trainingPrevWeek').onclick=()=>{trainingWeekOffset--;renderTrainings()};$('#trainingNextWeek').onclick=()=>{trainingWeekOffset++;renderTrainings()};$('#trainingTodayWeek').onclick=()=>{trainingWeekOffset=0;renderTrainings()};$$('[data-training-view]').forEach(b=>b.onclick=()=>{trainingViewMode=b.dataset.trainingView;$$('[data-training-view]').forEach(x=>x.classList.toggle('active',x===b));renderTrainings()});$('#chargeMonthFilter').onchange=renderCharges;$('#chargeCategoryFilter').onchange=renderCharges;$('#chargeStatusFilter').onchange=renderCharges;
