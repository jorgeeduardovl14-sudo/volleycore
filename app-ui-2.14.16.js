...
let profile=null,playerPrivateById=new Map(),players=[],categories=[],venues=[],events=[],announcements=[],charges=[],sinpeReports=[],onvoPayments=[],shirtRequests=[],allUsers=[],familyUsers=[],trainerUsers=[],families=[],seasons=[],trainingSeries=[],trainingExceptions=[],linkRequests=[],categoryVideos=[],attendanceRecords=[],notifications=[],exitRecords=[];
...
const PILOT_VERSION='UI 2.14.16';
...
function renderDashboard(){
  ...
  const upcomingMatches=events.filter(e=>e.type==='match'&&e.date>=today()&&e.status!=='cancelled').sort((a,b)=>`${a.date} ${a.startTime||''}`.localeCompare(`${b.date} ${b.startTime||''}`)).slice(0,5);
  ...
  $('#dashboardNextMatch').innerHTML=upcomingMatches.length?`
    <div class="vc-dashboard-match-list">${upcomingMatches.map(m=>`<div class="vc-dashboard-match-row"><strong>${esc(catName(m.categoryId))}</strong><span>${m.opponent?`vs ${esc(m.opponent)}`:esc(m.title)}</span><small>${dashboardFormatDate(m.date)} · ${esc(m.startTime||'Hora por definir')} · ${esc(eventLocationName(m))}</small></div>`).join('')}</div>
    <span class="vc-card-link">Ver todos los partidos →</span>
...
const TRAINING_CATEGORY_PALETTE=['#2563eb','#7c3aed','#db2777','#ea580c','#059669','#0891b2','#4f46e5','#65a30d','#c026d3','#dc2626'];
function trainingColor(categoryId){const ordered=[...categories].sort((a,b)=>(a.name||'').localeCompare(b.name||''));const i=Math.max(0,ordered.findIndex(c=>c.id===categoryId));return TRAINING_CATEGORY_PALETTE[i%TRAINING_CATEGORY_PALETTE.length]}
...
function trainingFilteredMatches(from,to){const cat=$('#trainingCategoryFilter').value,se=$('#trainingSeasonFilter').value,venue=$('#trainingVenueFilter').value;return events.filter(e=>e.type==='match'&&e.status!=='cancelled'&&e.date>=from&&e.date<=to&&(!cat||e.categoryId===cat)&&(!se||e.seasonId===se)&&(!venue||e.venueId===venue))}
function renderTrainingLegend(list){const ids=[...new Set(list.map(t=>t.categoryId).filter(Boolean))];$('#trainingLegend').innerHTML=ids.map(id=>`<span class="training-legend-item"><i style="--training-color:${trainingColor(id)}"></i>${esc(catName(id))}</span>`).join('')+`<span class="training-legend-item training-match-legend">🏆 Partido</span>`}
function renderTrainingMonth(list){
  ...
  const trainingOcc=list.flatMap(t=>occurrences(t,from,to).filter(o=>o.status!=='cancelled').map(o=>({...o,series:t,kind:'training'})));
  const matches=trainingFilteredMatches(from,to).map(e=>({kind:'match',date:e.date,startTime:e.startTime||'',event:e}));
  const entries=[...trainingOcc,...matches].sort((a,b)=>`${a.date} ${a.startTime||''}`.localeCompare(`${b.date} ${b.startTime||''}`));
  ...
  $('#trainingMonthCalendar').innerHTML=`<div class="training-month-weekdays">${['Lun','Mar','Mié','Jue','Vie','Sáb','Dom'].map(x=>`<strong>${x}</strong>`).join('')}</div><div class="training-month-grid">${days.map(date=>{const inMonth=date.slice(0,7)===isoDateLocal(md).slice(0,7),items=entries.filter(x=>x.date===date);return`<section class="training-month-day ${inMonth?'':'outside-month'}"><header>${Number(date.slice(8,10))}</header><div class="training-month-items">${items.map(x=>{if(x.kind==='match'){const e=x.event;return`<button type="button" class="training-month-item match" style="--training-color:${trainingColor(e.categoryId)}" data-edit-event="${e.id}"><b>🏆 ${esc(catName(e.categoryId))}</b><span>${esc(e.startTime||'')} ${e.opponent?'· vs '+esc(e.opponent):''}</span></button>`}const t=x.series;return`<button type="button" class="training-month-item training" style="--training-color:${trainingColor(t.categoryId)}" data-edit-training="${t.id}"><b>🏐 ${esc(catName(t.categoryId))}</b><span>${esc(x.startTime)} · ${esc(venueName(x.venueId||t.venueId))}</span></button>`}).join('')}</div></section>`}).join('')}</div>`;
}
function renderTrainings(){
  ...
  const weekOcc=list.flatMap(t=>occurrences(t,ws,we).filter(o=>o.status!=='cancelled').map(o=>({...o,series:t}))),weekMatches=trainingFilteredMatches(ws,we);
  ...
  $('#trainingStats').innerHTML=[[upcoming,'Próximos entrenamientos','upcoming'],[weekOcc.length,'Esta semana','week'],[activeCats,'Categorías activas','categories'],[hours.toFixed(hours%1?1:0),'Horas programadas','hours']].map(([v,l,a])=>`<button type="button" class="stat clickable-stat training-stat-card" data-training-stat="${a}"><strong>${v}</strong><span>${l}</span></button>`).join('');
  ...
  if(trainingViewMode==='week'){
    ...
    $('#trainingCalendar').innerHTML=names.map((name,i)=>{const date=datePlus(ws,i),items=[...weekOcc.filter(o=>o.date===date).map(o=>({kind:'training',...o})),...weekMatches.filter(e=>e.date===date).map(e=>({kind:'match',startTime:e.startTime||'',event:e}))].sort((a,b)=>(a.startTime||'').localeCompare(b.startTime||''));return`<section class="training-day"><header><strong>${name}</strong><span>${date.slice(8,10)}/${date.slice(5,7)}</span></header><div class="training-day-body">${items.map(x=>{if(x.kind==='match'){const e=x.event;return`<button class="training-block training-match-block" style="--training-color:${trainingColor(e.categoryId)}" data-edit-event="${e.id}" type="button"><span>🏆 PARTIDO</span><strong>${esc(catName(e.categoryId))}</strong><span>${esc(e.startTime||'Hora por definir')}${e.opponent?' · vs '+esc(e.opponent):''}</span><small>${esc(eventLocationName(e))}</small></button>`}const t=x.series;return`<button class="training-block" style="--training-color:${trainingColor(t.categoryId)}" data-edit-training="${t.id}" type="button"><span class="training-category-dot"></span><strong>${esc(catName(t.categoryId))}</strong><span>${esc(x.startTime)} – ${esc(x.endTime)}</span><small>${esc(venueName(x.venueId||t.venueId))}</small>${x.status==='changed'?'<em>Modificado</em>':''}</button>`}).join('')||'<span class="training-empty">Sin entrenamientos ni partidos</span>'}</div></section>`}).join('');
