...
let profile=null,players=[],categories=[],venues=[],events=[],announcements=[],charges=[],sinpeReports=[],allUsers=[],familyUsers=[],trainerUsers=[],families=[],seasons=[],trainingSeries=[],trainingExceptions=[],linkRequests=[],categoryVideos=[];
...
async function loadAdminData(){const [cs,ps,vs,es,as,chs,srs,us,ss,ts,te,lrs,fs,cvs]=await Promise.all([
...
function categoryEventsList(id){return events.filter(e=>e.categoryId===id).sort((a,b)=>(a.date||'').localeCompare(b.date||''))}
function categoryTrainingsList(id){return trainingSeries.filter(t=>t.categoryId===id&&t.status==='active')}
function categoryVideosList(id){return categoryVideos.filter(v=>v.categoryId===id).sort((a,b)=>(b.date||'').localeCompare(a.date||''))}
function categoryDetailHTML(id,familyMode=false){const c=categories.find(x=>x.id===id);if(!c)return'<p class="muted">Categoría no encontrada.</p>';const roster=(familyMode?familyPlayers:players).filter(p=>playerCatIds(p).includes(id)),trainings=categoryTrainingsList(id),games=categoryEventsList(id),videos=categoryVideosList(id);return`<div class="category-detail-grid"><article class="panel"><span class="eyebrow">INFORMACIÓN</span><h4>${esc(c.name)}</h4><p><strong>Entrenador:</strong> ${esc(c.coach||'—')}<br><strong>Asistente:</strong> ${esc(c.assistant||'—')}<br><strong>Temporada:</strong> ${esc(seasonName(c.seasonId))}<br><strong>Sede:</strong> ${esc(venueName(c.venueId))}</p>${venueLinks(c.venueId)}</article><article class="panel"><span class="eyebrow">RESUMEN</span><h4>${roster.length} jugadoras</h4><p>${trainings.length} entrenamiento(s)<br>${games.length} partido(s) / evento(s)<br>${videos.length} video(s)</p></article></div><section class="category-section"><h4>Entrenamientos</h4>${trainings.map(t=>`<article class="panel"><strong>${esc(daysLabel(t.days||[]))}</strong><p>${esc(t.startTime||'')} – ${esc(t.endTime||'')} · ${esc(venueName(t.venueId))}</p></article>`).join('')||'<p class="muted">No hay entrenamientos configurados.</p>'}</section><section class="category-section"><h4>Calendario de juegos y eventos</h4>${games.map(e=>`<article class="panel"><div class="page-head compact-head"><div><span class="eyebrow">${esc(typeLabel(e.type))}</span><h4>${esc(e.title)}</h4></div><span class="badge ${e.status}">${esc(statusLabel(e.status))}</span></div><p><strong>${esc(e.date)}</strong> · ${esc(e.startTime||'')} ${e.endTime?'– '+esc(e.endTime):''}</p><p>${e.opponent?`Rival: ${esc(e.opponent)} · `:''}${esc(venueName(e.venueId))}</p></article>`).join('')||'<p class="muted">No hay partidos o eventos registrados.</p>'}</section><section class="category-section"><h4>Videos de partidos</h4>${videos.map(v=>`<article class="panel category-video-card"><div><span class="eyebrow">${esc(v.date||'')}</span><h4>${esc(v.title)}</h4><p>${v.opponent?`Rival: ${esc(v.opponent)} · `:''}${esc(v.tournament||'')}</p><a class="category-link" href="${esc(v.url)}" target="_blank" rel="noopener">Abrir video</a><p class="muted">${esc(v.notes||'')}</p></div>${familyMode?'':`<div class="actions"><button class="action" data-edit-category-video="${v.id}">Editar</button><button class="action danger-action" data-delete-category-video="${v.id}">Eliminar</button></div>`}</article>`).join('')||'<p class="muted">No hay videos registrados.</p>'}</section><section class="category-section"><h4>Jugadoras</h4>${roster.map(p=>`<div class="person-row"><span class="person-icon">🏐</span><span><strong>${esc(p.name)}</strong><small>${esc(p.playerCode||'')} ${p.number?`· #${esc(p.number)}`:''}</small></span></div>`).join('')||'<p class="muted">No hay jugadoras vinculadas.</p>'}</section>`}
function openCategoryDetail(id){const c=categories.find(x=>x.id===id);if(!c)return;$('#categoryDetailTitle').textContent=c.name;$('#categoryDetailContent').innerHTML=categoryDetailHTML(id,false);$('#addCategoryVideoButton').dataset.categoryId=id;$('#addCategoryVideoButton').classList.remove('hidden');$('#categoryDetailDialog').showModal()}
function openFamilyCategoryDetail(id){const c=categories.find(x=>x.id===id);if(!c)return;$('#categoryDetailTitle').textContent=c.name;$('#categoryDetailContent').innerHTML=categoryDetailHTML(id,true);$('#addCategoryVideoButton').classList.add('hidden');$('#categoryDetailDialog').showModal()}
function openCategoryVideo(v=null,categoryId=''){$('#categoryVideoForm').reset();$('#categoryVideoDocId').value=v?.id||'';$('#categoryVideoCategoryId').value=v?.categoryId||categoryId;$('#categoryVideoTitle').value=v?.title||'';$('#categoryVideoDate').value=v?.date||today();$('#categoryVideoOpponent').value=v?.opponent||'';$('#categoryVideoTournament').value=v?.tournament||'';$('#categoryVideoUrl').value=v?.url||'';$('#categoryVideoNotes').value=v?.notes||'';$('#categoryVideoDialog').showModal()}
function renderFamilyCategories(){const ids=[...new Set(familyPlayers.flatMap(p=>playerCatIds(p)))];$('#familyCategoriesList').innerHTML=ids.map(id=>{const c=categories.find(x=>x.id===id);if(!c)return'';const next=categoryEventsList(id).find(e=>e.date>=today()&&e.status!=='cancelled');return`<article class="panel"><span class="eyebrow">${esc(seasonName(c.seasonId))}</span><h3>${esc(c.name)}</h3><p><strong>Entrenador:</strong> ${esc(c.coach||'—')}</p><p>${next?`Próximo evento: <strong>${esc(next.date)} · ${esc(next.title)}</strong>`:'Sin próximos eventos registrados.'}</p><button class="btn primary" data-family-category="${id}">Ver categoría</button></article>`}).join('')||'<p class="muted">No tienes categorías vinculadas todavía.</p>'}
$('#categoryVideoForm').onsubmit=async e=>{e.preventDefault();try{const id=$('#categoryVideoDocId').value,data={orgId:ORG_ID,categoryId:$('#categoryVideoCategoryId').value,title:$('#categoryVideoTitle').value.trim(),date:$('#categoryVideoDate').value,opponent:$('#categoryVideoOpponent').value.trim(),tournament:$('#categoryVideoTournament').value.trim(),url:$('#categoryVideoUrl').value.trim(),notes:$('#categoryVideoNotes').value.trim(),updatedAt:serverTimestamp()};if(id)await updateDoc(doc(db,'categoryVideos',id),data);else await addDoc(collection(db,'categoryVideos'),{...data,createdAt:serverTimestamp()});$('#categoryVideoDialog').close();await loadAdminData();openCategoryDetail(data.categoryId);toast('Video guardado.')}catch(x){alert(err(x))}};
$('#addCategoryVideoButton').onclick=()=>openCategoryVideo(null,$('#addCategoryVideoButton').dataset.categoryId);
