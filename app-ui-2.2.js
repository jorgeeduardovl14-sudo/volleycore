...
function openEvent(e){$('#eventForm').reset();$('#eventDocId').value=e?.id||'';$('#eventCategory').value=e?.categoryId||categories[0]?.id||'';$('#eventSeason').value=e?.seasonId||seasons.find(s=>s.isCurrent)?.id||seasons[0]?.id||'';$('#eventType').value=e?.type==='training'?'match':(e?.type||'match');$('#eventTitle').value=e?.title||'';$('#eventOpponent').value=e?.opponent||'';$('#eventDate').value=e?.date||today();$('#eventStart').value=e?.startTime||'';$('#eventEnd').value=e?.endTime||'';$('#eventHomeAway').value=e?.homeAway||'';$('#eventVenue').value=e?.venueId||'';$('#eventLocationUrl').value=e?.locationUrl||'';$('#eventStatus').value=e?.status||'scheduled';$('#eventNotes').value=e?.notes||'';$('#eventDialogTitle').textContent=e?'Editar evento':'Nuevo evento';$('#eventDialog').showModal()}
...
$('#editEventFromDetail').onclick=e=>{const id=e.currentTarget.dataset.eventId;$('#eventDetailDialog').close();openEvent(events.find(x=>x.id===id));};
