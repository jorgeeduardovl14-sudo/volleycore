...
function syncRoleManagerFields(){
  const role=$('#roleManagerRole')?.value,coaching=['trainer','assistant'].includes(role);
  $('#roleManagerCategoriesField')?.classList.toggle('hidden',!coaching);
  $('#roleManagerPermissionsField')?.classList.toggle('hidden',!coaching);
  $('#roleManagerPlayerNote')?.classList.toggle('hidden',role!=='player');
}
...
$('#roleManagerForm').onsubmit=async e=>{
  ...
  const coaching=['trainer','assistant'].includes(newRole);
  const assignedCategoryIds=coaching?$$('#roleManagerCategories input:checked').map(i=>i.value):[];
  if(coaching&&!assignedCategoryIds.length)return alert('Selecciona al menos una categoría para este perfil deportivo.');
  const linkedPlayers=players.filter(p=>(p.linkedUserIds||[]).includes(uid));
  if(newRole==='player'&&!linkedPlayers.length&&!confirm('Este usuario todavía no está vinculado a una ficha de jugadora. ¿Deseas asignar el rol Jugadora de todos modos?'))return;
  ...
  const data={role:newRole,accountType,status,assignedCategoryIds,updatedAt:serverTimestamp()};
