...
const topLogout=$('#logoutButton');if(topLogout)topLogout.onclick=()=>signOut(auth);$('#resendVerificationButton').onclick=async()=>{try{await sendEmailVerification(auth.currentUser);toast('Verificación reenviada.')}catch(x){alert(err(x))}};
