...
let profile=null,players=[],categories=[],venues=[],events=[],announcements=[],charges=[],sinpeReports=[],onvoPayments=[],shirtRequests=[],allUsers=[],familyUsers=[],trainerUsers=[],families=[],seasons=[],trainingSeries=[],trainingExceptions=[],linkRequests=[],categoryVideos=[],attendanceRecords=[],notifications=[];
...
async function loadFamilyData(){
  ...
  familyPlayers=isPlayer?(ownPlayer?[ownPlayer]:[]):fps;
...
function renderFamilyCharges(){
  ...
  $('#familyChargesList').innerHTML=list.map(c=>{
    ...
    const payButton=remaining>0&&!['paid','exempt'].includes(c.status)?`<button class="btn primary onvo-pay-button" data-pay-onvo="${c.id}" type="button">Pagar con ONVO · Prueba</button>`:'';
...
function showApp(user,p){
  ...
  if(admin){go('dashboard');loadAdminData().then(()=>{startAdminRealtime();startNotificationRealtime();loadShirtRequests().then(startShirtRealtime);if(!state.familiesReconciled&&families.length===0&&players.some(x=>(x.linkedUserIds||[]).length)){state.familiesReconciled=true;reconcileFamilies(false).catch(console.error)}}).catch(e=>alert(err(e)))}
  else if(coachingStaff){go('trainerHome');loadTrainerData().then(()=>{startNotificationRealtime();loadShirtRequests().then(startShirtRealtime);}).catch(e=>alert(err(e)));}
  else if(pendingStaff){const label=p.role==='pendingAssistant'?'asistente':'entrenador';$('#trainerPendingStatus').textContent=p.status==='rejected'?'Solicitud no aprobada':'Pendiente de aprobación';$('#trainerPendingTitle').textContent=p.status==='rejected'?`Solicitud de ${label}`:`Solicitud de ${label} pendiente`;go('trainerPending');startNotificationRealtime();loadShirtRequests().then(startShirtRealtime);}
  else{state.familyDataLoading=true;go('familyHome');loadFamilyData().then(()=>{startFamilyRealtime();startNotificationRealtime();loadShirtRequests().then(startShirtRealtime);handleOnvoReturn();}).catch(e=>alert(err(e)));const playerMode=isPlayerProfile(p);const payBtn=$('[data-view="familyPayments"]');if(payBtn)payBtn.classList.toggle('hidden',playerMode);const linkBtn=$('[data-view="familyLink"]');if(linkBtn)linkBtn.classList.toggle('hidden',playerMode);}
...
$('#shirtProduct').onchange=()=>{$('#shirtPlayerNameField').classList.toggle('shirt-player-required',$('#shirtProduct').value==='initiation_uniform');};
$('#shirtRequestForm').onsubmit=async e=>{
  ...
  try{
    ...
    $('#shirtRequestForm').reset();$('#shirtQuantity').value=1;await loadShirtRequests();toast('Solicitud enviada correctamente.');
  }catch(x){alert(err(x))}
};
...
$('#userAdminStatusFilter').onchange=renderUserAdmin;document.addEventListener('click',e=>{if(!e.target.closest('#paymentReportMonthsBox'))togglePaymentMonthsMenu(false);});updatePaymentReportStatusOptions();updatePaymentReportPeriodControls();
...
state.adminListeners.push(onSnapshot(org('charges'),snap=>{charges=snap.docs.map(d=>({id:d.id,...d.data()}));if(state.currentView==='finances')renderFinances();if(state.currentView==='sinpeAdmin'){renderSinpeAdmin();}},console.error));
