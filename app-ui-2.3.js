...
function trainerEventCard(e){
  ...
  return `<article class="panel trainer-event-card" data-trainer-event="${e.id}" role="button" tabindex="0">
    ...
    <p class="muted">${esc(venueName(e.venueId))}${locationUrl?' · Ubicación disponible':''}</p>
  </article>`;
}
...
function openEventDetail(id){
  ...
  $('#editEventFromDetail').classList.toggle('hidden',!['admin','treasurer'].includes(profile?.role));
