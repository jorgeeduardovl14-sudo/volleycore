...
function notificationCategoryIds(){
  ...
  if(isCoachingRole(profile?.role))return profile.assignedCategoryIds||[];
  return [...new Set((familyPlayers||[]).flatMap(p=>playerCatIds(p)))];
}
...
$('#eventForm').onsubmit=async e=>{
  ...
  try{
    ...
    await createCategoryNotification({kind:'event',categoryId,title:id?'Evento actualizado':`Nuevo ${typeLabel(data.type).toLowerCase()}`,body:`${data.title} · ${data.date}${data.startTime?' · '+data.startTime:''}`,sourceId:eventId});
    $('#eventDialog').close();
