...
const PILOT_VERSION='UI 2.14.17';
...
function eventDisplayIcon(e){if(e.type==='festival')return '🎪';return e.homeAway==='away'?'🚌':'🏠'}
function eventDisplayLabel(e){return e.type==='festival'?'FESTIVAL':(e.homeAway==='away'?'VISITA':'CASA')}
...
function renderTrainings(){
  ...
  if(trainingViewMode==='week'){
    ...
    $('#trainingCalendar').innerHTML=names.map((name,i)=>{const date=datePlus(ws,i),items=[...weekOcc.filter(o=>o.date===date).map(o=>({kind:'training',...o})),...weekEvents.filter(e=>e.date===date).map(e=>({kind:'event',startTime:e.startTime||'',event:e}))].sort((a,b)=>(a.startTime||'').localeCompare(b.startTime||''));return`<section class="training-day"><header><strong>${name}</strong><span>${date.slice(8,10)}/${date.slice(5,7)}</span></header><div class="training-day-body">${items.map(x=>{if(x.kind==='event'){const e=x.event;return`<button class="training-block training-match-block ${e.type==='festival'?'training-festival-block':''}" style="--training-color:${trainingColor(e.categoryId)}" data-edit-event="${e.id}" type="button"><span>${eventDisplayIcon(e)} ${eventDisplayLabel(e)}</span><strong>${esc(catName(e.categoryId))}</strong><span>${esc(eventTimeLabel(e))}${e.opponent?' · vs '+esc(e.opponent):''}</span><small>${esc(eventLocationName(e))}</small></button>`}const t=x.series;return`<button class="training-block" style="--training-color:${trainingColor(t.categoryId)}" data-edit-training="${t.id}" type="button"><span class="training-category-dot"></span><strong>${esc(catName(t.categoryId))}</strong><span>${esc(x.startTime)} – ${esc(x.endTime)}</span><small>${esc(venueName(x.venueId||t.venueId))}</small>${x.status==='changed'?'<em>Modificado</em>':''}</button>`}).join('')||'<span class="training-empty">Sin entrenamientos ni partidos</span>'}</div></section>`}).join('');
